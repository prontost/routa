...
def _conn() -> sqlite3.Connection:
    ...
    cols = {r[1] for r in con.execute("PRAGMA table_info(work_money_accounts)")}
    if "currency" not in cols:
        con.execute("ALTER TABLE work_money_accounts ADD COLUMN currency TEXT DEFAULT 'KRW'")
    if "label" not in cols:
        con.execute("ALTER TABLE work_money_accounts ADD COLUMN label TEXT")
    if "tenant" not in cols:
        con.execute("ALTER TABLE work_money_accounts ADD COLUMN tenant INTEGER NOT NULL DEFAULT 1")
    return con


def set_label(account: str, label: str) -> None:
    with _conn() as con:
        con.execute("UPDATE work_money_accounts SET label=? WHERE tenant=? AND account=?",
                    (label.strip(), _tid(), account))


def account_label(account: str) -> str | None:
    with _conn() as con:
        row = con.execute("SELECT label FROM work_money_accounts WHERE tenant=? AND account=?",
...
def register(account: str, kind: str = "other", ord: int = 0,
    ...
    with _conn() as con:
        con.execute(
            "INSERT INTO work_money_accounts (tenant, account, kind, ord, currency) VALUES (?,?,?,?,?) "
...
def set_currency(account: str, currency: str) -> None:
    with _conn() as con:
        con.execute("UPDATE work_money_accounts SET currency=? WHERE tenant=? AND account=?",
                    (currency.upper(), _tid(), account))


def unregister(account: str) -> None:
    with _conn() as con:
        con.execute("DELETE FROM work_money_accounts WHERE tenant=? AND account=?", (_tid(), account))
...
def registered() -> dict[str, str]:
    ...
    with _conn() as con:
        return {r[0]: r[1] for r in
                con.execute("SELECT account, kind FROM work_money_accounts "
...
def registered_full() -> dict[str, dict]:
    with _conn() as con:
        rows = con.execute(
            "SELECT account, kind, ord, currency FROM work_money_accounts "
            "WHERE tenant=? ORDER BY ord, account", (_tid(),)).fetchall()
...
def account_currency(account: str) -> str:
    with _conn() as con:
        row = con.execute("SELECT currency FROM work_money_accounts WHERE tenant=? AND account=?",
                          (_tid(), account)).fetchone()
