...
_SCHEMA = """
CREATE TABLE IF NOT EXISTS work_entry_meta (
...
def _conn() -> sqlite3.Connection:
    ...
    existing = {r[1] for r in con.execute("PRAGMA table_info(work_slept_entries)")}
    for col, dtype in wanted.items():
        if col not in existing:
            con.execute(f"ALTER TABLE work_slept_entries ADD COLUMN {col} {dtype}")
    con.execute("CREATE INDEX IF NOT EXISTS idx_slept_account ON work_slept_entries(tenant, account)")
    for tbl in ("work_entry_meta",):
...
def set_occurred(voucher: str, occurred_at: str) -> None:
    with _conn() as con:
        con.execute(
            "INSERT INTO work_entry_meta (tenant, voucher, occurred_at) VALUES (?,?,?) "
...
def occurred_map(vouchers: list[str]) -> dict[str, str]:
    ...
    with _conn() as con:
        rows = con.execute(
            f"SELECT voucher, occurred_at FROM work_entry_meta WHERE tenant=? AND voucher IN ({ph})",
...
def sleep_record(account: str, snap: dict, occurred_at: str | None) -> None:
    with _conn() as con:
        con.execute(
            "INSERT INTO work_slept_entries (tenant, account, debit, credit, amount, posting_date, remark, occurred_at) "
            "VALUES (?,?,?,?,?,?,?,?)",
...
def sleeping_for(account: str) -> list[dict]:
    with _conn() as con:
        rows = con.execute(
            ...
            "FROM work_slept_entries WHERE tenant=? AND account=?", (_tid(), account)).fetchall()
...
def clear_sleeping(account: str) -> None:
    with _conn() as con:
        con.execute("DELETE FROM work_slept_entries WHERE tenant=? AND account=?", (_tid(), account))


def forget(voucher: str) -> None:
    with _conn() as con:
        con.execute("DELETE FROM work_entry_meta WHERE tenant=? AND voucher=?", (_tid(), voucher))
